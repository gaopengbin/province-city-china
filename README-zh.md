...
仓库中提供了一个无需构建的示例页面：`examples/map-export.html`。
...
- `http://localhost:3000/examples/map-export.html`（使用 `serve` 时）
- `http://localhost:8080/examples/map-export.html`（使用 `python` 时）
...
- 中国地图脚本自动多源加载（unpkg / npmmirror / jsDelivr），降低单一 CDN 失效导致黑屏的概率。
